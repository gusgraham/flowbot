...
import { Plus, Trash2, MapPin, AlertTriangle, Droplets, Ruler } from 'lucide-react';
import {
...
export default function MonitorsTab({ projectId }: MonitorsTabProps) {
    ...
    const [newMonitor, setNewMonitor] = useState({ name: '', icm_node_reference: '', is_depth_only: false });
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        setNewMonitor({ name: '', icm_node_reference: '', is_depth_only: false });
    ...
    const toggleDepthOnly = (monitor: VerificationFlowMonitor) => {
        updateMonitor.mutate({
            monitorId: monitor.id,
            update: { is_depth_only: !monitor.is_depth_only },
            projectId
        });
    };
    ...
    const sortedMonitors = React.useMemo(() => {
        if (!monitors) return [];
        return [...monitors].sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }));
    }, [monitors]);
    ...
    return (
        <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="flex items-center pt-6">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={newMonitor.is_depth_only}
                                        onChange={(e) => setNewMonitor({ ...newMonitor, is_depth_only: e.target.checked })}
                                        className="w-4 h-4 text-green-600 rounded border-gray-300 focus:ring-green-500"
                                    />
                                    <span className="text-sm font-medium text-gray-700">Depth Verification Only</span>
                                </label>
                            </div>
                        </div>
            ...
            {/* Monitors List */}
            {
                sortedMonitors && sortedMonitors.length > 0 ? (
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Depth Only</th>
                                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white divide-y divide-gray-200">
                                {sortedMonitors.map((monitor: VerificationFlowMonitor) => (
                                    <tr key={monitor.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <button
                                                onClick={() => toggleDepthOnly(monitor)}
                                                className={`inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full ${monitor.is_depth_only
                                                    ? 'bg-purple-100 text-purple-700'
                                                    : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                                                    }`}
                                            >
                                                <Ruler size={12} />
                                                {monitor.is_depth_only ? 'Yes' : 'No'}
                                            </button>
                                        </td>
                                        <td className="px-4 py-3 text-center">
                                            <button
