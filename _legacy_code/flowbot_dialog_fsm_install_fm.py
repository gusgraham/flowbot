...
class flowbot_dialog_fsm_install_fm(QtWidgets.QDialog, Ui_Dialog):

    def __init__(self, parent=None):
        """Constructor."""
        super(flowbot_dialog_fsm_install_fm, self).__init__(parent)
        self.setupUi(self)
        ...
        removed_height = self.vlo_rg_data.sizeHint().height()

        self.remove_rg_widgets()

        self.setFixedHeight(original_height - removed_height)
        self.install_sheet: Optional[bytes] = None
        # self.resize_form()

    def remove_rg_widgets(self):
        # Remove all layouts in vlo_fm_data
        while self.vlo_rg_data.count():
            layout_item = self.vlo_rg_data.takeAt(0)
