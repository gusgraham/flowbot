...
from ui_elements.ui_flowbot_dialog_fsm_review_pumplogger_base import Ui_Dialog

class flowbot_dialog_fsm_review_pumplogger(QtWidgets.QDialog, Ui_Dialog):
    ...
    def __init__(self, interim_id: int, a_project: fsmProject, parent=None):
        """Constructor."""
        super(flowbot_dialog_fsm_review_pumplogger, self).__init__(parent)
        self.setupUi(self)
        ...
        for a_inst in self.a_project.dict_fsm_installs.values():
            if a_inst.install_type == 'Pump Logger':
                a_int_pl = self.a_project.get_interim_review(interim_id=interim_id, install_id=a_inst.install_id)
                if not a_int_pl:
                    a_int_pl = fsmInterimReview()
                    a_int_pl.interim_review_id = self.a_project.get_next_interim_review_id()
                    a_int_pl.interim_id = interim_id
                    a_int_pl.install_id = a_inst.install_id
                    self.a_project.add_interim_review(a_int_pl)
                self.interim_reviews.append(a_int_pl)
        ...
        self.plot_axis_onoff: Optional[axes.Axes] = None
        self.plot_axis_other_depth: Optional[axes.Axes] = None
        ...
        self.plotCanvasReviewPL.figure.set_dpi(100)
        self.plotCanvasReviewPL.figure.set_figwidth(self.fig_width)
        self.plotCanvasReviewPL.figure.set_figheight(self.fig_height)

        self.btnPrevious.clicked.connect(self.on_previous_clicked)
        self.btnNext.clicked.connect(self.on_next_clicked)
        # self.tabWidget.currentChanged.connect(self.onTabChanged)

        # self.chk_ fdv_plot_rg.clicked.connect(self.update_plot)
        self.chk_onoff_full_period.clicked.connect(self.update_plot)
        self.chk_onoff_compare_full_period.clicked.connect(self.update_plot)
        self.cbo_onoff_other_install.currentIndexChanged.connect(self.update_plot)
        # self.cbo_onoff_other_install.currentIndexChanged.connect(
        #     self.on_other_install_combobox_changed
        # )
        ...
        self.update_plot()
        self.plotCanvasReviewPL.scrollZoomCompleted.connect(self.update_stats)
    ...
    # def on_other_install_combobox_changed(self):
    #     pass

    def update_widgets(self):
        if len(self.interim_reviews) > 0:
            self.chk_pl_review_complete.setChecked(self.current_interim_review.pl_complete)
            self.txt_review_comments.setText(self.current_interim_review.pl_comment)
    ...
    def update_plot(self):
        ...
        self.plotCanvasReviewPL.figure.clear()
        self.plotCanvasReviewPL.scroll_zoom_enabled = True
        self.plotCanvasReviewPL.x_pan_enabled = True
        self.create_pl_plot()
    ...
    def update_stats(self):
        ...
        x_min, x_max = self.plot_axis_onoff.get_xlim()
        ...
        mask = (self.df_filtered['Date'] >= start_time) & (self.df_filtered['Date'] <= end_time)
        df_visible = self.df_filtered.loc[mask]

        if df_visible.empty:
            stats_str = "No data in view"
        else:
            # Ensure the data is sorted by time.
            df_visible = df_visible.sort_values('Date')

            # Convert the time column to a NumPy array for easier arithmetic.
            times = df_visible['Date'].to_numpy()
            values = df_visible['OnOffData'].to_numpy()
            ...
            if len(values) > 0:
                if values[0] == 1:
                    ...
                    if times[0] != self.df_filtered['Date'].iloc[0]:
                        n_off += 1                
            ...
            if len(times) > 0:
                ...
                if values[0] == 0 and self.df_filtered['Date'].iloc[0] < times[0]:
            ...
            if len(times) > 0:
                ...
                if values[-1] == 1 and self.df_filtered['Date'].iloc[-1] > times[-1]:
            ...
            data_start = self.df_filtered['Date'].min()
            data_end   = self.df_filtered['Date'].max()
        ...
        # Update the text box
        self.stats_box.set_text(stats_str)
        # Redraw the canvas for the update to appear
        self.plot_axis_onoff.figure.canvas.draw_idle()

    def create_pl_plot(self):

        if len(self.interim_reviews) == 0:
            ax = self.plotCanvasReviewPL.figure.subplots()
            ax.text(0.5, 0.5, 'No Pump Logger data in the interim', horizontalalignment='center', verticalalignment='center', fontsize=16)
            ax.set_axis_off()
            return
        ...
        if self.chk_onoff_compare_install.isChecked():
            (self.plot_axis_other_depth, self.plot_axis_onoff) = self.plotCanvasReviewPL.figure.subplots(
                nrows=2, sharex=True, gridspec_kw={'height_ratios': [1, 1]})
        else:
            (self.plot_axis_onoff) = self.plotCanvasReviewPL.figure.subplots(
                nrows=1, sharex=True, gridspec_kw={'height_ratios': [1]})

        if self.chk_onoff_compare_full_period.isChecked():

            self.plot_axis_onoff.step(
                self.df_other_filtered["Timestamp"],
                self.df_other_filtered["Value"],
        ...
        self.plot_axis_onoff.set_ylabel("On/Off")
        self.plot_axis_onoff.set_title("Pump Logger", loc="left", fontsize=16)
        locator = mpl_dates.AutoDateLocator(minticks=6, maxticks=15)
        formatter = mpl_dates.ConciseDateFormatter(locator)
        self.plot_axis_onoff.xaxis.set_major_locator(locator)
        self.plot_axis_onoff.xaxis.set_major_formatter(formatter)

        # Create a statistics text box in the top left (in axis coordinates)
        self.stats_box = self.plot_axis_onoff.text(0.02, 0.98, "", transform=self.plot_axis_onoff.transAxes,
            fontsize=10, verticalalignment="top", bbox=dict(facecolor="white", alpha=0.5, edgecolor="gray"))
        ...
        self.plot_axis_onoff.callbacks.connect('xlim_changed', lambda ax: update_stats(ax))

        # Call update_stats once to initialize the statistics box.
        self.update_stats()

        # self.plotCanvasReviewPL.figure.subplots_adjust(left=0.15, right=0.85, bottom=0.15, top=0.85)
        # self.plotCanvasReviewPL.figure.tight_layout()

        # Adjust layout and redraw the figure
        self.plotCanvasReviewPL.figure.tight_layout()
        self.plotCanvasReviewPL.figure.subplots_adjust(left=0.075, right=0.95, bottom=0.05, top=0.95)
        self.plotCanvasReviewPL.figure.canvas.draw() 
    ...
    def update_interim_review(self):
        if len(self.interim_reviews) > 0:        
            self.current_interim_review.pl_complete = self.chk_pl_review_complete.isChecked()
            self.current_interim_review.pl_comment = self.txt_review_comments.text()
    ...
    def filter_pl_data(self):

        if self.chk_onoff_full_period.isChecked():
        ...
        if self.chk_onoff_compare_install.isChecked():
            other_install_id = self.cbo_onoff_other_install.currentText()
            a_inst = self.a_project.dict_fsm_installs[other_install_id]

            if self.chk_onoff_full_period.isChecked():
                self.df_other_filtered = a_inst.data.copy()
            else:    
                self.df_other_filtered = a_inst.data[(a_inst.data['Date'] >= self.start_date) & (a_inst.data['Date'] <= self.end_date)].copy()
