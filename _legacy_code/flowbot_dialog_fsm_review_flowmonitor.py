...
class flowbot_dialog_fsm_review_flowmonitor(QtWidgets.QDialog, Ui_Dialog):
    ...
    def __init__(self, interim_id: int, a_project: fsmProject, parent=None):
        ...
        for a_inst in self.a_project.dict_fsm_installs.values():
            if a_inst.install_type != 'Rain Gauge':
                ...
                if a_int_fm.dr_data_covered:
                    self.interim_reviews.append(a_int_fm)
                else:
                    a_int_fm.fm_complete = True
                    a_int_fm.fm_comment = 'No data in interim period'
        ...
        if len(self.interim_reviews) > 0:
            ...
            self.current_inst: fsmInstall = self.a_project.dict_fsm_installs[self.current_interim_review.install_id]
        else:
    ...
    def update_widgets(self):
        ...
        if self.current_interim_review is not None:
            ...
            self.chk_fm_review_complete.setChecked(True)
